...
public:
  CarrotBroadcaster() : Node("carrot_broadcaster") {
    ...
    phase_  = this->declare_parameter<double>("phase", 0.0);           // ← NEW
    if (dir_ == 0) dir_ = 1;

    broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(this);

    last_time_ = now();
    angle_ = phase_;                                                   // ← стартуем со сдвигом
    timer_ = this->create_wall_timer(50ms, std::bind(&CarrotBroadcaster::onTimer, this));

    RCLCPP_INFO(this->get_logger(),
      "Carrot '%s' around '%s': R=%.3f, dir=%d, omega=%.3f, phase=%.3f",
      child_frame_.c_str(), parent_frame_.c_str(), radius_, dir_, omega_, phase_);
  }
...
private:
  ...
  double radius_{2.0}, omega_{0.6}, angle_{0.0}, phase_{0.0};
  int dir_{1};
};
